...
class RawNet(nn.Module):
    ...
    def summary(self, input_size, batch_size=-1, device="cuda"):
        ...
        print("----------------------------------------------------------------")
        print("Input size (MB):", self._sizeof(input_size))
        print("Forward/backward pass size (MB):", self._sizeof(x))
        print("Params size (MB):", self._sizeof(summary))
        print("Estimated Total Size (MB):", self._sizeof(input_size) + self._sizeof(x) + self._sizeof(summary))
        print("----------------------------------------------------------------")
        return
    ...
    def _sizeof(self, data):
        ...
        if isinstance(data, torch.Tensor):
            ...
            for d in data:
                total_size += self._sizeof(d)
        elif isinstance(data, dict):
            for key, value in data.items():
                total_size += self._sizeof(key)
                total_size += self._sizeof(value)
        else:
